...
engine side of this is built (a real port/edge/cycle-checked graph) and the
authoring surface is not** — the only pipeline-authoring UI that exists is still
a linear chain, built with `itertools.pairwise`, the exact pattern this
paragraph names as the failure. This is not a design gap to argue about; it is
the next real work, and it is graph-shaped work, not a linear one extended.
