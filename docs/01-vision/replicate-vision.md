The replicate vision is that replicates almost certainly are independent - having a tab to split the replicates just makes sense. This isolates the replicates for what they are, and optionally the user can inherit the settings from an already configured replicate.
...
Replicates are independent, and do not share a processing frame, so there are no seam concerns.
...
Calibration is a replicate property, not a filter property and not a
source property. Physical scale, working scale, and any per-experiment
...
## 7. Source ownership — batch and HPC

The source video owns its replicates. Batch and HPC operations against
a source implicitly run every replicate spawned from that source. Each
