...
nothing else — same params, same view, same output.
...
holds *intent only* — never progress.
...
"which steps are complete" is a query rather than a stored flag.
...
handful of peephole rules added when profiling shows a path is hot.
...
choice never touches configuration.
