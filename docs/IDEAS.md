...
I added an overview.md - my idea is that this can be an autogenerated document, which I then utilize to write the readme. But I think to do this, we need to add a bunch of stuff as special formatting around functions, or at the very least special formatting at the top of files.

> Grumpy Sr. Engineer Opinion: `docs/.overview.md` is zero bytes. You are designing a generation format for a document you have not written one paragraph of by hand, which is how you get a generator that emits what is easy to generate rather than what you wanted. And the leading dot in this repo means *generated* — `.state.md`, `.index.md` — so hand-adding `.overview.md` already misfiled it.
>
> Mentor Sr. Engineer Opinion: The "special formatting around functions" you are reaching for is docstrings. You have them, pyright strict keeps their signatures honest, and CLAUDE.md already routes interface contracts to them. A second annotation layer is a second home for the same fact, which is the one doc rule this repo enforces hardest.
>
> Assessment: A README needs three things and only one is per-function. What SIEVE is; what it does *today versus intends*, which already exists as SCAFFOLD's Built/Projected split and is the only honest what-exists-now boundary in the tree; and how to run it, which is `noxfile.py` plus the entry points. Neither of the first two comes from function annotations. The one generatable part is the module inventory, and its source should be module docstring first lines — the same source and the same tool as idea 7. Write `tools/module_index.py` once and feed both SCAFFOLD's Built annotations and an overview inventory block from it.
>
> Bottom line: Write the README by hand first and see which paragraphs you wished were generated; that list will be shorter than you expect and it will be the inventory. No new comment syntax — docstring first line is the annotation. Delete the empty `.overview.md` until something generates it.
