...
| Project selector | The same stack surface as the pipeline — library card above, one `ChainCard` per project, selection accent, ↑/↓ move it, double-click enters. Not a platform list widget. A card names the project and carries the step card's `→` (enter it) and `✕` (out of the library, the folder untouched; disabled on the last one) in its head, what it holds under that, and when it was last opened at its foot — the one line that is read and never acted on sits where reading ends. The library card carries NEW PROJECT (mint an empty project, the selection lands on it without entering the pipeline) and the selected card alone carries OPEN LOCATION beside the last-opened line, both in the timeline button's dress | `build_project_pane`, `_project_card`, `_open_project_button`, `_close_project_button`, `_chrome_button`, `add_project`, `close_project`, `Control.new_project`, `Control.close_project`, `_reveal_project` |
