...
src/sieve/cli/sweep_cmd.py                 # `sieve sweep` — decode throughput over core sets and worker counts.
...
src/sieve/bench/sweep.py                   # Sweep a cost over core sets and worker counts, so a constant can be judged.
...
scripts/mutation_sweep.py                  # Mutation sweeps over one file: unique anchors, byte-exact restore, a green bounded baseline.
```
