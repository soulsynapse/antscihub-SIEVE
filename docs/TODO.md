...
- Right click on the video in the filter tab should take it back to the replicate tab full view (noticed <=2026.07.27) → `todo/click-through-navigation.md`, which rejects the right-click binding in favour of an output boundary in the chain-stack. Folded in there 2026.07.28; it had been its own item holding only that rejection.
...
- If the user sets the replicate and tries to change it, it should ask for confirmation as a box. (noticed <=2026.07.27) → `todo/lock-a-visited-replicate.md`. Sharpened 2026.07.27: a replicate that has been opened in the filter tab is locked; the edit is refused, the cost is stated, and accepting performs the move.
...
- `nox -s checks` can fail on a budget miss that is ambient load, not a regression — `density_rebuild` came in at 101.7 ms against a 100 ms limit on a docs-only tree and passed three times in isolation straight after (noticed 2026.07.28) → `todo/budget-checks-under-ambient-load.md`. The cause is a single-shot `perf_counter` at the edge of the legal range running under `--benchmark-disable`; the sweep for other single-shot budget checks in the gate is part of that item.
