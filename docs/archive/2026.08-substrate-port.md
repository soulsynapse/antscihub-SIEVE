# Substrate port — the plan in phases (walked)

**Archival. Nothing here is current state.** This was the route from the
experiment folders into `src/sieve/`, written to be walked and to leave the
tree when the last phase landed. They all landed. It is kept instead of
deleted because the walking turned up more than the code did: what was
decided and why, what the explorers had wrong, and a dozen defects that a
reader planning the next stretch would otherwise rediscover.

Read it as a record of a journey and never as a description of the tree. What
is true now is in the code and in
`experiments/substrate-checks/`, where each phase's properties are asserted
and each has a `--broken` mode that shows the check can fail. Where this file
says a thing "will" happen or is "still owed", it is describing the state of
play on the day that line was written, and several of those are done.
...
out, and nothing was deleted from them to mark a phase done.

## What outlived it

Worth promoting into something durable if the next stretch leans on them,
and left here rather than minted unprompted:

- **Decisions that are still load-bearing** — the window stores the crop and
  a nudge pays a re-decode; retention consults residency so no form needs its
  own budget; the ladder returns an ordered list of attempts rather than one
  choice; verification extends the harness pattern rather than adding a
  second notion of evidence.
- **Defects the checks caught**, each of which a later reader would otherwise
  find the hard way: the proxy maps to its source by timestamp and not by
  row; a half-frame tolerance hands back frame 20 for every row of the head;
  chunked grey came back contrast-squeezed against memory; two series keys
  wrote to one file; the residency set and the resident store spelled their
  key in opposite orders so nothing was ever protected; a batch's last
  segment was never published; a display form sized to a pane would key a
  stored frame to the window it was looked at in; and the same rounding rule,
  written twice, disagreed with itself by two pixels.
- **What is built and not yet driven** — a proxy builder nothing starts, a
  declaration layer nothing declares into, a waste count with one caller.
...
*Was not covered at the time, and is now:* `05-provenance.py`'s invariant —
that a stored value is reproducible from the key it is filed under — could
not be run against `src/sieve` because nothing in the port wrote a series.
`sieve.analysis.record` and `experiments/substrate-checks/11-provenance.py`
closed that. It runs against the fake route, so the invariant is established
for synthetic frames and has still never been run over a real decode.
