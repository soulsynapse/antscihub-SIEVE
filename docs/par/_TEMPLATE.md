...
<!-- THE TEMPLATE. Copy to `docs/par/NNNN-short-title.md`, fill the slots,
delete each comment block as you fill it. A finished PAR contains no
`<!--` at all. Every section carries the test it has to survive, stated
at the top of its comment in bold. Prose that survives none of them is
the freewheeling this form exists to stop.
...
derivation. That is why every example in these comments is indented and
the stamp placeholder above is not a well-formed stamp — a comment does
not hide a line from either derivation, only indentation does. In a real
PAR the frontmatter `Status:` and the marker both sit at column 0 and
mean what they say. -->

<!-- THE TITLE, three parts. Is the name clear to a user — does it
...
**Plain language first**, throughout the file. Repo-specific terms and
citations go in parentheses, so what lands here does not inherit the
session's vocabulary. Do not simplify — keep it accurate and drop the
jargon, which are different operations. -->

<!-- THE MARKER, stubs only. A system owed a rationale is a stub at its
own number: this skeleton with a marker in place of the body, one
physical line at column 0, immediately below the title.
...
fidelity is honest the day it is filed — from the sitting that filed it,
never from a later guess. `Expensive` may be a list of the questions
that will be expensive. Everything else stays empty.
...
error this template was written after. It does not stop another document
asserting into unclaimed territory. It makes the territory greppable
while it is still unsettled, so the assertion is visible rather than
silent. -->
...
<!-- THE EPIGRAPH. **Test: is this Kendrick's, verbatim?**

His own statement of what the system is for, blockquoted, attributed and
dated.
...
absent, deleted along with this comment. An agent filling it with
synthesized prose in his voice is the worst failure this system can
produce, worse than an empty slot, because a reread cannot tell it from
the real thing. -->
...
the source passages and the decision's **original** date. It reports the
decision as made and never improves it — daylight between the
...
<!-- **Test: does anything cross this domain's edge? If nothing does,
say why — do not omit the section.**

What may pass in, what may pass out, and what may never pass either way.

Some domains have no traffic: a convention, a rationale class, a naming
rule. Those state that they have none and why, because an absent section
is ambiguous — a reader cannot tell "none" from "forgotten" — while a
stated absence is evidence, the same reason a placeholder is the debt
entry rather than a note about one. -->
...
Where the distinction carries weight, use the verbs consistently and say
so — *declares* under rules held elsewhere, *interprets* values it did
not author, *owns* only what it may change. PAR-0007 is the worked
example. -->

## Evidence

<!-- **Test: for each system cited, what transfers and what does not?
One cited without that is decoration.**
...
A claim that can be added later without touching what already exists has
not earned admission here, however true and however important. It is a
citizen of the domain and goes below, or it belongs to another domain
and goes there, or it waits. A rationale that settles cheap claims
alongside expensive ones settles an empty room, and the cheap ones are
exactly what turns out wrong once something real exists to argue
against. -->

## Cheap

<!-- **Test: can this move without anything else having to move with
it?**

The domain's citizens: what lives inside it and does the work.

Cheap is not a verdict on importance — it is a statement about mobility,
and it is why the rationale can govern without these being settled. A
cheap internal is often more central to daily use than anything above
it. It is admitted to the domain on the same terms; what it has not done
is earn admission to *Expensive*, and the only thing that earns that is
...
inventory somebody sat down to write.** Someone asked whether this lived
in the domain and the answer was yes; that exchange is the entry.
Without the trigger the section has no terminal form and grows by
whim. -->
...
A bequest is provisional and must read that way. The receiving rationale
may refuse it on evidence this one did not have, and refusal amends this
rationale rather than being overridden by it. A bequest to a stub goes
to a file that cannot answer yet — write it as a question the stub will
settle, not a ruling it will inherit.
...
<!-- **Test: does each one name something that can be done, or checked,
by somebody who is not you?**

Two classes, because a generic outcome accepts any prose while these two
do not.
...
later proposals against the system are judged by, which is why they must
be operable rather than admirable. -->

## Why this boundary

<!-- **Test: what does it cost to not do it this way?**

The closing argument, in two parts: what is paid by drawing the boundary
somewhere else or not at all, and why this seam is a natural one rather
than a line someone chose.

Short. If it cannot be made short, the boundary is not yet
understood. -->

## Challenges

<!-- **Test: does this doubt break the decision? If it does, it is not
an entry — the rationale is rewritten.**
...
- Friction is stated, never inferred. An agent may point it out; a human
  confirms before it lands, and *Raised by* is there so a reread sees at
  a glance which entries survived that.
...
three times in a year — is re-litigated from scratch every time it lands
nowhere. -->

<!-- BEFORE ACCEPTING. Acceptance means the rationale governs and the
roll-up is owed that moment — `ARCHITECTURE.md` for architecture,
`README.md` or `AGENTS.md` for repo mechanisms, amended in the same
commit. A `Proposed` rationale amends nothing, which is what makes it
free to sit.

Four checks, each catching a failure that has actually happened here:

- Every claim in `Expensive` states its reversal cost. Any that does not
  is in the wrong section.
- Nothing in `Expensive` is there because it is important. Importance is
  not the filter; reversal cost is.
- Neither Edges flag fires.
- The reader finishes the file convinced without opening another. A
  decision split across three cross-citing files makes the reader do the
  assembly, which is the work this tier exists to have already done. -->
