...
The kind list was deferred and is ratified (2026-08-08). PLAN named SetParam,
DrawRegion, SetSpan, AddNode, and two settled decisions had hollowed half of
it since it was written: `adr/one-field-is-one-populated-value.md` makes a
span one pair-shaped param, and `adr/gui-knows-kinds-not-tools.md` makes a
drawn overlay an editor bound to a param field "entering through the same
command path as a typed value". So DrawRegion and SetSpan named emitting
surfaces, not document mutations — as kinds they would key the layer by which
widget produced the value, which is the coupling the layer exists to
dissolve. The list enumerates mutations of the saved file instead, and
PLAN.md's Phase 7 now states it:
...
its Phase-7 citation carries that clause now, so the definition and the layer
cannot drift apart.
