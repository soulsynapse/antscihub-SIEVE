...
A Proposed record governs nothing: whatever governed before keeps
...
pressure to accept comes only from wanting the record to govern.
...
can be made directly when the record is small, its claims are cheap to
check, and doubt has had time to arrive organically.

Convene a deliberate attack instead when any of these hold: the record
must govern before organic challenge could accumulate; it was authored
and judged by the same eyes in one sitting; its necessity argument
rests on empirical claims nobody has verified; or its value is not
clear — the record feels load-bearing but cannot yet say why. An
attack is never owed (PAR-0001); it is convened.
...
- Verify the empirical claims yourself rather than accepting them,
  and say what the record loses if a claim fails ("the Context loses
  its footing and the Decision is left standing on much narrower
  ground").
...
the PAR; the frozen record keeps its error (Exchange 1).

Check: the primary exists, `Status: Open`, before the verdict lands.

## 3. What every PAR must survive

The floor — checks any record faces at judgment, attack or not:

- Every citation resolves, and measures what the record says it
...
- The central sentence supports the central examples. Read the
  Decision's strongest rule against the record's own evidence; a
  standard that forbids what the record exists to enable, or licenses
  it only through an unstated convention, is a hole (Exchange 3).
- One named system, near-decomposable: dense inside, sparse across,
  seams to neighbouring records stated as citations. Simulate the
  decision reversing — if a sibling record needs substantial
...
- Concessions are typed correctly: each Challenges entry narrowed to
  what actually stands, neither carrying a concession the record does
  not owe (Exchange 6) nor holding a doubt that in fact breaks.

Check: each bullet answered against the tree, not from memory.

## 4. What a good PAR could have

The instruments for a record whose value is not clear. Not every
record needs these — but they are what moved PAR-0005 from "coherent
draft nobody could weigh" to good enough for acceptance, and each was
passed back as a question at its judgment:
...
- **The value named, and the no-other-route argument** (Exchange 8):
  ...
  record's keep; claims that do not survive it ("speed") are struck
  from the Context so the record never leans on them.
...
- **The skeleton test** (Exchange 9): draft the how-tos the record
  claims to admit, each with its check step. A check step that cannot
  be stated is the cheapest detector of a hole in the record above it
  — at PAR-0005's judgment, exactly the two unwritable check steps
...
whole-record (PAR-0001's coherence rule), reviewed as a diff, and the
primary freezes at the ruling. The record stays Proposed; tier 1 keeps
citing the old regime (Exchange 12's close).

Check: the primary is Frozen; the record still greps as Proposed.

## 6. The acceptance commit

Its own sitting. All of it lands in one commit, or in adjacent commits
chunked records-then-placements (AGENTS.md):

- Flip the status line; the tier-1 surface that cites the record is
...
- `grep -rln "PAR-NNNN"` over the tree: every citation checked
  against the record's *new* body. A citation pointing at text the
  record no longer contains is a mismatch fixed at or before
  acceptance; if the dropped text carried a benefit still wanted, its
  new home is named (Exchange 5).
- Markers and placeholders quoting the record narrow to its accepted
  wording; `python -m sieve.debt write`; the regenerated ledger
  travels in the same commit.
- Anything the record routes to siblings is stated in their text or
