"""Two tabs, Pipeline and Step, and the current-step rail down the left
edge outside both — switching tabs must never hide it, since it tracks
...
from PySide6.QtWidgets import QHBoxLayout, QListWidget, QListWidgetItem, QTabWidget, QWidget
...
class PipelinePanel(QWidget):
    def __init__(self, pipeline: Pipeline, parent: QWidget | None = None) -> None:
        ...
        self._tabs.addTab(QWidget(self._tabs), "Pipeline")
        self._tabs.addTab(self._build_step_list(pipeline), "Step")
    ...
    def _build_step_list(self, pipeline: Pipeline) -> QWidget:
        step_list = QListWidget(self._tabs)
