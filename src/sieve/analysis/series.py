...
**Invalidation needs no machinery.** What a stored value depends on is
folded into the tool key and the form key, so a change upstream of the
series names a different series and a change downstream of it — a threshold
read off the numbers, a smoothing applied at display — names the same one
and correctly reuses it.

**Coverage here and coverage on disk are the same rule twice.** A frame tier
records which spans it holds rather than reading a directory
(`sieve.store.coverage`); this records which rows have a value rather than
reading a zero. Both exist because absence and a legitimate value are
indistinguishable in the data, and the consumer that has to know to check is
...
class Series:
    ...
    def save(self, root: Path) -> Path:
        ...
        The filename follows from that rather than fighting it. Substituting
        the offending characters out of the key looks like enough and is not:
        the substitution is many-to-one, so two keys differing only where it
        maps land on one file and the second silently destroys the first —
        with the sidecar left honestly describing the survivor, which is the
        worst possible outcome because nothing is detectably wrong afterwards.
        So the stem is a readable prefix for a human browsing the directory
        plus a digest of the whole key for uniqueness, and no part of it is
        ever parsed. Same arrangement as `sieve.store.coverage`, for the same
        reason.
        """
        with self.lock:
            values, covered = self.values.copy(), self.covered.copy()
        root.mkdir(parents=True, exist_ok=True)
        path = root / f"{_stem(self.key)}.npz"
...
def _stem(key: str) -> str:
    """A filename for a key: readable enough to browse, unique by digest.

    The prefix is lossy on purpose and nothing reads it back; the digest is
    what makes two keys two files. Truncated so that a long tool key cannot
    push the path past a platform limit.
    """
    readable = re.sub(r"[^A-Za-z0-9._-]+", "-", key).strip("-")[:80]
    fingerprint = hashlib.blake2b(key.encode("utf-8"), digest_size=8).hexdigest()
    return f"{readable}-{fingerprint}" if readable else fingerprint
