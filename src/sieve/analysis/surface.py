...
widget that blits them is `sieve.gui`'s business, and keeping the split means
the reductions can be measured without a GUI — which is also why they live
under `analysis` rather than beside the widget: what a series looks like
reduced to a strip is a fact about the series, and a copy of it inside a view
would be the second place that was decided.
"""
