...
`budgets.py` declares twelve latency ceilings. Until this module, none of them
could be checked by anything, because nothing in the repo emitted a timing —
and a budget nothing can miss is not a budget. This is the one place a duration
...
the only thing in the repo knowing both this and Qt.
...
#: somewhere else is responsible for getting there. A Qt adapter doing exactly
#: that (a queued signal emission) is why the bus does not try.
...
class MetricBus:
    ...
    a player's scrub round trip, which its own degradation policy needs anyway.
    """
...
class Recorder:
    ...
    def median_ms(self, key: str) -> float:
        ...
        Median rather than mean: one pathological sample — a page fault, a GC
        pause, a scheduler decision — should not be able to fail a gate, and the
        thing a budget describes is the typical interaction rather than the
        worst one ever observed.

        Raises:
            KeyError: if nothing was recorded under `key`. A gate asserting on
                an empty series would pass vacuously, which is the failure a
                benchmark has to refuse at the collection level.
        """
    ...
    def misses(self) -> tuple[Sample, ...]:
        ...
        each consumer: this is one pass over what arrived, not a second
