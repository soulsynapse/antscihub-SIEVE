...
met them. Neither can answer the question `mutual/shares.py`'s worker constants
raise, which is not "what is the optimum here" but **how sharp is it** — a flat
optimum makes a controller a failure mode with no upside, and a sharp one makes
a per-machine constant wrong everywhere it was not measured. That is a property
of a response surface, and a point measurement cannot state it however carefully
the point is taken.
...
core configurations this hardware does not have, which is how one machine
answers a question about several. It is the cores axis only: four cores masked
out of a large machine keep that machine's whole last-level cache and memory
controller, so nothing here samples a genuinely low-bandwidth machine, and the
constant this is aimed at (`mutual/shares.py` `PREVIEW_WORKERS`) is justified as
a *bandwidth* property. A sweep that forgot this would report the strongest
possible evidence for the weakest of its axes.
...
class Reading:
    ...
    def best(self) -> float:
        ...
        `bench/metrics.py`'s `median_ms` argues the choice between this and
        `typical` at length and the argument is the same one here; a sweep
        reports both because which claim is being made is the *caller's*
        question, and a surface that offered one would have decided it for them.
        """
...
def sweep(
    ...
    **Interleaved, not blocked, and this is the whole reason the function
    exists rather than a nested loop at the call site.** Blocked runs of v2's
    density surface on its reference machine read 82-87 ms for eight readings
