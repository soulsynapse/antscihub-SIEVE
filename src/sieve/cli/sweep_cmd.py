...
The instrument for the question `mutual/shares.py` arrived carrying: its caps
and shares were chosen on v2's machine and nothing since has asked whether they
hold here. v2's luma finding
(`docs/findings/2026.07.28-the-luma-path-has-almost-nothing-left-to-thread.md`,
in the v2 tree) ran this sweep by hand, on one core set, and its protocol is
reproduced here so the two are comparable: sequential reads over a fixed span, a
warm-up pass, repeats, and the median reported per configuration. What is added
is the machine axis — the same measurement under a restricted set of cores,
which is how one machine answers a question about several.
...
`run_cmd.frame_source` is deliberately not what this opens the footage through.
That helper exists to keep `luma` out of a user's hands — it is `not
Dag.needs_chroma` for the graph about to run — and it takes no worker count at
all, because a run's count is the resolver's. Both are exactly what a sweep has
to vary, so it builds its own `PrefetchFrameSource`: not a second decode path
(there is one reader class and it is that one), but the same reader driven at
the two settings the resolver would otherwise have decided.
"""
...
from sieve.cli.run_cmd import refuse
from sieve.decode.prefetch import PrefetchFrameSource
from sieve.decode.reader import VideoDecodeError

#: v2's luma finding's span, so a reading here can be compared with the readings
...
def sweep_decode(
    ...
    def objective(cell: Cell) -> float:
        with PrefetchFrameSource(video, workers=cell.workers, luma=luma) as reader:
