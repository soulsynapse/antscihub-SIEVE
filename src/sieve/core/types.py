...
class ROI:
    ...
    def __post_init__(self) -> None:
        if self.width <= 0 or self.height <= 0:
            raise ValueError(
                f"ROI must have positive extent, got {self.width}x{self.height}"
            )
        if self.x < 0 or self.y < 0:
            raise ValueError(
                f"ROI origin must be non-negative, got ({self.x}, {self.y})"
            )
    ...
    def clamped_to(self, width: int, height: int) -> ROI:
        ...
        return ROI(
            x=left, y=top, width=max(right - left, 1), height=max(bottom - top, 1)
        )
    ...
    def placed_in(
        ...
        if frame is None:
            return cls(
                x=max(x, 0), y=max(y, 0), width=max(width, 1), height=max(height, 1)
            )
    ...
    def resized_in(
        self, width: int, height: int, frame: tuple[int, int] | None
    ) -> Self:
