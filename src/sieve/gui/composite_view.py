            ...
            for b in range(blocks):
                ...
                if self.fill_alpha > 0.0:
                    ...
                    if right >= x0 + 1 and bottom >= y0 + 1:
                        painter.fillRect(
                            QRect(x0 + 1, y0 + 1, right - x0, bottom - y0), fill
                        )
...
class StepCompositeView(QWidget):
    ...
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        self._line_slider = self._grid_slider(
            DEFAULT_LINE_STEP, "Detected border alpha"
        )
        ...
        self._grid_tags = tuple(
            self._tag_label(text) for text in ("fill", "border", "heat")
        )
