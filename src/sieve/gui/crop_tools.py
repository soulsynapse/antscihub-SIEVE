...
class CropToolsPanel(QWidget):
    ...
    def __init__(
        self, document: ReplicateDocument, parent: QWidget | None = None
    ) -> None:
        super().__init__(parent)
    ...
    def _build_tools_box(self) -> QGroupBox:
        ...
        self._fit_button.setToolTip(
            "Return to the fitted view. Scroll on the video to magnify."
        )
        zoom = QHBoxLayout()
    ...
    def set_mode(self, mode: str) -> None:
        button = (
            self._draw_button if CropMode(mode) is CropMode.DRAW else self._stamp_button
        )
    ...
    def _on_stamp_edited(self) -> None:
        if self._refreshing:
            return
        self.stamp_size_changed.emit(
            self._stamp_width.value(), self._stamp_height.value()
        )

    @Slot()
    def _on_set_all_clicked(self) -> None:
        self.set_all_requested.emit(
            self._stamp_width.value(), self._stamp_height.value()
        )
