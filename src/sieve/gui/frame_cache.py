...
Bounded by bytes rather than by count. Proxy frames are ~2.8 MB at 1280x720
but the proxy width is a preference and the source aspect ratio is not ours to
assume, so a count-based cap would be a memory ceiling that moves when the
user changes an unrelated setting.

Playback frames are deliberately *not* cached by the player. Playback walks
the whole timeline and would evict everything a scrub had warmed, which is
exactly backwards — the frames worth keeping are the ones the user returned to.
"""
