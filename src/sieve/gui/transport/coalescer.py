...
This is the ordering discipline that makes a scrub feel bounded. In v2, a burst
of 40 seeks settled on the final target in two decodes where queueing the same
burst took ~3.2 s and painted 38 frames nobody asked for
(`docs/V2-MAP.md`). The player holds the scrub budget by discarding work, not by
decoding faster.

Qt-free, like `scrub_policy.py` and `pacing.py`, and for the same reason: the
properties worth pinning are orderings, and orderings are tested by feeding them
numbers.
...
(`request_intent.RequestKind`). Where a guess and a commitment compete for the
one pending slot the commitment wins regardless of arrival order. Between
...
viewport and cached there. `generation` is that stamp — the same mechanism
`decode/prefetch.py` names as borrowed from here.
"""
