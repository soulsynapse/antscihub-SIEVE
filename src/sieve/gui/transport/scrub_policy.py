...
The scrub budget cannot be met by decoding faster. Measured in v2 on the
reference source, a random seek was ~68 ms of which ~47 ms was the container
seek before a single pixel was converted, and hardware acceleration does not
engage in this OpenCV build. There is no tuning left. On a machine slower than
the reference there is less than none.
...
class ScrubPolicy:
    ...
    def set_allow_degrade(self, allowed: bool) -> None:
        ...
        next drag: a user turning this off is asking to see the effect now, not
        eventually.
        """
