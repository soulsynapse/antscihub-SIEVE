...
in a single-state gallery they would look like arguments about a border.
...
from sieve.gui.view.dev.card_mockups.look import LOOKS, Look, MockCard
...
def _row(left: QWidget, right: QWidget) -> QWidget:
    ...
    cards.addWidget(left)
    cards.addWidget(right)
    cards.addStretch(1)
...
def _real_pair() -> QWidget:
    ...
    Its title and rows are what a view fills, so the gallery fills them with the
    step every mock is drawn holding — a baseline showing different content
    would make every difference below ambiguous between look and content.
    """
...
def _real_card(selected: bool) -> Card:
    ...
    for line in ("sensitivity — 0.42", "min area — 120 px"):
        label = QLabel(line)
