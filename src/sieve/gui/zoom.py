...
This lives apart from any widget because two of them now need it, and the
clamp is the part that costs a day to re-derive. `VideoView` maps widget
points to *source pixels* and `_CompositePane` maps them to *blocks*, so what
they share is not a coordinate system but the rule for producing one: hence
the centre and `at` speak in **normalized content coordinates** — [0, 1] over
the painted rectangle — and each widget scales that into its own units. A
pane with a block grid and no frame has no pixel dimensions at all, so a
shared helper denominated in source pixels would have had nothing to say to
it.
