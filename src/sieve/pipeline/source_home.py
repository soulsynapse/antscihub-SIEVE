...
A `CropArtifact` is unreadable without all three. `project_dir` is what its
...
it against nothing, and one holding `identity` without `video` learns a record
is stale with nothing to re-cut from. They travel together or the caller has a
bug that looks like a working call.
...
It lived as `gui.document.SourceHome` until it was moved. The docstring there
justified itself by pointing at `ResolvedSource`, one layer down and out of
reach, which was the tell — and `run`, `preview`, and `detect` were each
re-deriving the same three fields by hand, including the same message for the
same failure, because the type that named them was above all three.
"""
...
class SourceHome:
    ...
    #: once there is one, and the conventional home beside the video until
    #: then, so an artifact written before the first save is not made
    #: unfindable by the save.
    project_dir: Path
    #: `cache_key.source_identity(video)`, held rather than derived on demand:
    #: it stats the parent, and the four-state reading is asked for far more
    #: often than the footage changes.
    identity: str
    ...
    def for_video(cls, video: Path, project_dir: Path) -> SourceHome:
        ...
        Raises:
            OSError: if the footage is not where the project says. Left to the
                ...
                refuses with a message naming the path, and the GUI drops the
                home entirely so that no record can claim to be at rest.
        """
