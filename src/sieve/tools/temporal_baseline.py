...
leaves no per-block baseline over time. This tool is that denominator.
...
holding the same ring and each computing the same median, so one of them could
hand over the MAD. Recomputing internally is one ring and one median. The
two-port version becomes right the day something other than this tool's own
numerator wants the baseline — which is also when `accepts` learns about ports
at all (`core/tool_base.py`).
...
**Why this tool is what made warmup params-derived.** Its lead-in is
...
#: plausible frame rate is already the largest decode any tool asks for.
...
#: What the input may be. Float only, and narrowly so: this tool holds up to
...
def window_frames(window_seconds: float, fps: float) -> int:
    ...
    all denominated in it, and a second copy is how a tool warms for a window
...
class BaselineState:
    ...
    `None` until the first frame, for `BlockSignalState`'s reason: nothing tells
...
def run(params: TemporalBaselineParams, window: FrameSpan, state: BaselineState, /) -> Frame:
    ...
    if params.emit is Emit.BASELINE:
        ...
        # the estimate out into a frame that changes under a result the GUI is
        # still painting — several frames after the edit that caused it. One
        # small allocation on a diagnostic-only path buys that off.
...
@register_tool(
    ...
    param_stereotypes={
        "window_seconds": ParamStereotype.SCALAR_RANGE,
...
class TemporalBaselineParams(ParamsBase):
    ...
    #: Source frame rate, used only to convert the window into frames. Explicit
    #: for `block_signal.fps`'s reason: a `run` is pure and cannot ask the graph
    #: what the container's rate was, so whatever configures the node writes it
    #: from a value it already owns.
