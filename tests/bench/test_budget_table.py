"""`bench/budgets.py` must agree with the budget table in VISION.md.
...
The document is VISION.md rather than v2's ARCHITECTURE.md because v3's
ARCHITECTURE.md is generated from the ADRs and VISION owns the claims that
define built-correctly (CLAUDE.md). The table is the loop's claim as numbers,
so that is where the readable half lives.
"""
...
VISION = Path(__file__).resolve().parents[2] / "docs" / "VISION.md"
...
def parse_vision_budgets() -> dict[str, tuple[Regime, float]]:
    """Extract `label -> (regime, limit in ms)` from the vision document."""
...
def documented() -> dict[str, tuple[Regime, float]]:
    parsed = parse_vision_budgets()
    assert parsed, f"Parsed no budgets from {VISION} — the table format changed"
    return parsed
