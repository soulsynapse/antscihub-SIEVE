"""`FrameCache` is what makes coarse scrubbing free, so its eviction matters."""
...
class TestBasics:
    def test_a_miss_returns_none(self, qapp: object) -> None:
        del qapp
        assert FrameCache().get(7) is None
...
class TestEviction:
    def test_it_evicts_to_stay_within_capacity(self, frame_bytes: int) -> None:
        cache = FrameCache(capacity_bytes=frame_bytes * 3)
    ...
    def test_it_evicts_the_least_recently_used(self, frame_bytes: int) -> None:
        cache = FrameCache(capacity_bytes=frame_bytes * 2)
        cache.put(1, image())
        cache.put(2, image())
