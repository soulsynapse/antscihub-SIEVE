"""What this tool claims: a unit that transfers, and a window that means it.

Ported from v2's file of the same name, whose framing carries. The claims about
*where* state lives — one ring per run, never shared, never cached — belong to
...
contract and is tested in `test_tool_contract.py`. What is left here is what
only this tool can get wrong: that the deviation is a number about the block
rather than about the lighting, that the robust statistics are actually robust,
and that `window_seconds` is the parameter its guidance says it is.

What is new is the parity half, on 03.7's mechanism. One golden per `emit`,
because the two are two code paths that share only the ring: the deviation is
the standardization and its degenerate-spread repair, while the baseline is the
median handed out on its own. A golden of the deviation alone would leave the
diagnostic output — the one a user sets `window_seconds` by looking at — pinned
by nothing.
"""
...
from sieve.tools.temporal_baseline import (
    FPS_MAX,
    MAX_SAMPLES,
    SETTLED_EPSILON,
    WINDOW_SECONDS_MAX,
...
from sieve.tools.temporal_baseline import run as temporal_baseline_run
...
#: copy of the scheme.
GOLDEN_PREFIX = "temporal_baseline_40x8x8_w1_"

#: The window every golden was cut at, in seconds. A golden regenerated under a
...
REGENERATE = (
    ...
    "src/sieve/filters/temporal_baseline.py src/sieve/core/types.py && "
    'uv run --project ../antscihub-SIEVE-v2 python -c "'
    "import numpy as np; "
    "from sieve.core.types import ChannelSpec, Frame; "
    "from sieve.filters.temporal_baseline import BaselineState, Emit, TemporalBaselineParams, "
    "temporal_baseline_cpu; "
    "s = np.random.default_rng(11).normal(1.0, 0.1, (40, 8, 8)).astype(np.float32); "
    "s[:, 0, 0] = 5.0; s[35:, 0, 0] += 3.0; s[30:, 3, 4] += 3.0; "
    "last = lambda p: [temporal_baseline_cpu(Frame(data=d, index=i, channels=ChannelSpec.GRAY), "
    "p, st).data for st in [BaselineState()] for i, d in enumerate(s)][-1]; "
    f"[np.save('tests/goldens/{GOLDEN_PREFIX}' + e.value + '.npy', "
    f"last(TemporalBaselineParams(window_seconds={GOLDEN_WINDOW_SECONDS}, fps=30.0, emit=e))) "
    'for e in Emit]"'
)
...
def golden_series() -> npt.NDArray[np.float32]:
    ...
    Two events on two kinds of cell, because the deviation has two arithmetics
    and one fixture has to reach both: `(3, 4)` is an ordinary noisy cell with a
    positive MAD, and `(0, 0)` is constant across its whole window and therefore
    has a MAD of exactly zero — the degenerate case `_floored` answers. A series
    of noise alone would pin the division and leave the repair uncovered.
    """
    series = np.random.default_rng(11).normal(1.0, 0.1, (40, NY, NX)).astype(np.float32)
    series[:, 0, 0] = 5.0
    series[35:, 0, 0] += 3.0
    series[30:, 3, 4] += 3.0
    return series
...
def test_the_deviation_is_unchanged_by_a_gain_and_an_offset() -> None:
    """The load-bearing claim, and the reason the tool exists.
...
def test_events_do_not_inflate_the_spread_they_are_measured_against() -> None:
    ...
    A fifth of the window is event, which is the regime the tool is built for:
...
def test_the_declared_warmup_is_the_worst_case_over_the_legal_range() -> None:
    ...
    """
    assert SPEC.settling_epsilon == SETTLED_EPSILON
...
def test_each_emit_reproduces_the_v2_golden(emit: Emit) -> None:
    ...
    The last frame of `golden_series()` is the one worth pinning: it is past a
    full window on both cells, so neither number is about the ring still
    filling, and both events are inside their windows.
    """
    golden = np.load(GOLDENS / f"{GOLDEN_PREFIX}{emit.value}.npy")
    params = TemporalBaselineParams(window_seconds=GOLDEN_WINDOW_SECONDS, fps=FPS, emit=emit)
...
def test_the_goldens_are_two_different_arrays() -> None:
    ...
    They are in different units — the baseline is in the input's, the deviation
    is dimensionless — so an implementation that emitted the median down both
    branches would satisfy every inequality above and fail only here.
    """
...
def test_the_regeneration_command_names_every_golden() -> None:
    ...
    assert f"window_seconds={GOLDEN_WINDOW_SECONDS}" in REGENERATE
