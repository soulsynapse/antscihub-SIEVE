"""Generate `docs/todo/.index.md`, and answer what to take next.

One file per item, YAML frontmatter, two shapes. A *sequenced* item carries
`step:` — dotted numbers whose first component is the phase, so "02.3.1" is an
aside inserted between "02.3" and "02.4" without renumbering anything. A *pool*
item carries `priority:` instead — noticed work that can wait, attached to a
phase or to none. The number is the ordering, which is what lets `--next` be
the whole selection rule: the queue that runs the work never has to encode an
order, the repo holds it.

The index is derived, never edited, and `tests/docs/test_todo_index.py` fails
when it drifts from the files it describes — v2's discipline, cut to the
subset v3 uses (no primer, no settled table, no `after:` graph; the step
numbers are the ordering those existed to compute).

    uv run python tools/todo_index.py           # rewrite the index
    uv run python tools/todo_index.py --check   # exit 1 if stale
    uv run python tools/todo_index.py --next    # path of the next takeable item
"""
...
NOTICE = "<!-- Generated by tools/todo_index.py. Do not edit; rerun the tool. -->"
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"todo_index: {error}", file=sys.stderr)
        return 1
    ...
    index = TODO_DIR / INDEX_NAME
    content = render(items, phase_titles())
    current = index.read_text(encoding="utf-8") if index.exists() else None
    if current == content:
        return 0
    if args.check:
        print(f"todo_index: stale — run `uv run python {Path(__file__).name}`", file=sys.stderr)
        return 1
    index.write_text(content, encoding="utf-8")
    print(f"todo_index: wrote {index.relative_to(REPO)}")
    return 0
